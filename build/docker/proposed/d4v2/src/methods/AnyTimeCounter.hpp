...
template <class T>
class AnyTimeCounter : public MethodManager, public Counter<T> {
 private:
  bool optDomConst;
 ...
 public:
   ...
   */
  AnyTimeCounter(po::variables_map &vm, std::string &meth, bool isFloat,
    ...
    m_cache = CacheManager<T>::makeCacheManager(vm, m_problem->getNbVar(),
                                                m_specs, m_out);

    // init the clock time.
    initTimer();

    std::string threshold = vm["threshold"].as<std::string>();
    m_limitCount = T(threshold);
    std::cout << "c [CONSTRUCTOR] Threshold: " << threshold << '\n';
    ...
    void *op = Operation<T, T>::makeOperationManager(meth, isFloat, m_problem,
                                                     m_specs, m_solver, m_out);
    m_operation = static_cast<Operation<T, T> *>(op);
    m_out << "c\n";
  }  // constructor
  ...
  */
  T compute_(std::vector<Var> &setOfVar, std::vector<Lit> &unitsLit,
    ...
    if (m_isTimeOut) {
      std::vector<Var> tmp;
      return m_operation->manageTop(tmp);
    }
    m_isTimeOut = m_isTimeOut && m_timeOut < getTimer();
#endif
    ...
    if (nbComponent) {
      T count = 1;
      for (unsigned i = 0; i < freeVariable.size(); i++) count *= 2;
      ...
      for (int cp = 0; cp < nbComponent; cp++) {
        ...
        if (count > m_maxCount) {
          m_maxCount = count;
          std::cout << "c i " << m_maxCount << '\n';
          if (m_maxCount >= m_limitCount) {
            std::cout << "c The number of models is greater than the given "
                         "threshold\n";
            std::cout << "s SAT\n";
            exit(0);
          }
